...
const ownerIds = [7300602830]; // contoh chat_id owner 
    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "RYUNAMI1");
...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<b>— ( 👋 ) Olaa, ${username}</b>
 I'm Bot RyuNami Infinity Creat By GyzenLyoraa

<blockquote>•「 Information Bot 」•</blockquote>
━━━━━━━━━━━━━━━━━━━━
⌬ Creator : @GyzenVtx
⌬ Botname : RyuNami Infinity
...
bot.command("ckey", async (ctx) => {
  ...
  if (!args || !args.includes(",")) {
    return ctx.reply("✗ Falha\n\nExample :\n• /ckey Gyzen,30d\n• /ckey RyuNami,30d,puki", { parse_mode: "HTML" });
  }
...
app.get("/", (req, res) => {
  const filePath = path.join(__dirname, "RyuNami", "Login.html");
...
app.get("/execution", (req, res) => {
  try {
    ...
    const filePath = "./RyuNami/Login.html";
...
const executionPage = (
  ...
  const filePath = path.join(__dirname, "RyuNami", "RyuNami.html");
